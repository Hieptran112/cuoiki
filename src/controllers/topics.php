...
function getTopicLessons() {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT * FROM topic_lessons WHERE topic_id = ? AND is_active = 1 ORDER BY lesson_number");
        $stmt->bind_param("i", $topicId);
        $stmt->execute();
...
function submitTopicExercise() {
    ...
    try {
        ...
        // Lưu kết quả
        $stmt = $conn->prepare("
            INSERT INTO topic_exercise_results (user_id, lesson_id, exercise_id, selected_answer, is_correct)
            VALUES (?, ?, ?, ?, ?)
        ...
        ");
        $stmt->bind_param("iiisi", $userId, $lessonId, $exerciseId, $selectedAnswer, $isCorrect);
        $stmt->execute();
...
function updateTopicProgress($userId, $lessonId) {
    ...
    try {
        ...
        $stmt = $conn->prepare("
            ...
            LEFT JOIN topic_exercise_results ter ON te.id = ter.exercise_id AND ter.user_id = ?
            WHERE te.lesson_id = ?
        ");
